...
import { PageIndexEditor } from './page-index-editor'

interface PageIndexDetailLayoutProps {
  data: PageIndexData
  themes: ThemeSummary[]
  initialThemeId: string | null
}

export function PageIndexDetailLayout({
  data,
  themes,
  initialThemeId,
}: PageIndexDetailLayoutProps) {
  ...
  return (
    <>
      ...
      <section className="mx-auto w-full max-w-5xl px-4 sm:px-6 py-6 space-y-6">
        ...
        {data.ogImage?.source?.publicUrl && (
          <div
            className="relative w-48 overflow-hidden rounded-md border border-border"
            style={{ aspectRatio: '1200/630' }}
          >
            <Image
              src={data.ogImage.source.publicUrl}
              alt="OG image"
              fill
