...
} from '@/server/helpers/get-course-progress'
...
async function viewHandler(
  ...
  // Reason: hyphens in Next.js dynamic segments require bracket notation
  ...
  const { id: progressId, viewCount } = await upsertLessonProgress(
    userId,
    courseId,
    pageId,
  )

  return NextResponse.json({ progressId, viewCount })
}

export const POST = apiHandler(viewHandler)
