    ...
    <>
      <Icon
        className={cn(
          ...
          destructive
            ? 'text-destructive'
            : 'text-muted-foreground dark:group-hover:text-white dark:group-focus:text-white',
        )}
      />
      ...
      {showChevron && (
        <ChevronRight className="h-4 w-4 text-muted-foreground dark:group-hover:text-white dark:group-focus:text-white" />
      )}
    </>
  )
}
  ...
  return (
    <DropdownMenu open={open} onOpenChange={onOpenChange} modal={false}>
      <DropdownMenuTrigger asChild>
        <Button
          variant="ghost"
          className="relative h-10 w-10 rounded-full p-0 border-0 hover:ring-2 hover:ring-primary/20 transition-all"
        ...
        <DropdownMenuGroup className="p-1">
          {regularItems.map((item) => (
            <DropdownMenuItem
              ...
              className="group gap-3 py-2.5 px-3 cursor-pointer"
            >
  ...
  return (
    <Drawer open={open} onOpenChange={onOpenChange}>
      <DrawerTrigger asChild>
        <Button
          variant="ghost"
          className="relative h-11 w-11 rounded-full p-0 border-0 active:scale-95 transition-transform"
          aria-label="Open user menu"
        >
          ...
          <nav className="space-y-1" role="menu">
            {regularItems.map((item) => (
              <button
                ...
                className="group flex w-full items-center gap-4 rounded-xl px-4 py-4 text-left transition-colors hover:bg-muted active:bg-muted/80"
                role="menuitem"
              >
                <MenuItemContent
                  icon={item.icon}
...
export function AvatarMenu({
  ...
  const effectiveUser: UserInfo = isAuthenticated
    ? (user ?? { name: '', email: '', isAdmin: false })
    : { name: 'hi', email: '', isAdmin: false }
  ...
  const menuItems: MenuItem[] = isAuthenticated
    ...
    : [
        { label: 'Log In', icon: LogIn, onClick: () => router.push('/login') },
        {
  ...
  if (isAuthenticated && effectiveUser.isAdmin) {
