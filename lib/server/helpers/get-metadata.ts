...
import type { PageMetadataInput } from '@/types/page'
import { AppSettings } from './AppSettings'

export async function getPageMetadata(
  page?: PageMetadataInput | string | null,
): Promise<Metadata> {
  ...
  // Normalize: accept string (legacy/static) or PageMetadataInput partial
  ...
  // ─── OG Image ───────────────────────────────────────────
  const ogImages = settings?.ogImage
    ? [
        {
          url: settings.ogImage.source?.publicUrl,
