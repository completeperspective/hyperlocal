...
const AUTH_SESSION_NAME = process.env.AUTH_SESSION_NAME || 'web-app'
const AUTH_SESSION_SECRET =
  process.env.AUTH_SESSION_SECRET || randomBytes(32).toString('base64url')
const AUTH_SESSION_EXPIRY = process.env.AUTH_SESSION_EXPIRY || '86400'
    ...
    async start({ context, data }) {
      ...
      try {
        const user = await keystoneContext.sudo().query.User.findOne({
          ...
          query:
            'id name email isAdmin walletAddress profile { nickname description location isPublic image { source { publicUrl } } }',
        })
